...
static int isTrackballUsed = 1;
static int isMouseUsed = 1;
...
{
// __android_log_print(ANDROID_LOG_INFO, "libSDL", "nativeMouse action=%d pointerID=%d  x=%d  y=%d",action,pointerId,x,y);
...
// __android_log_print(ANDROID_LOG_INFO, "libSDL", "AKI %d %08X",action,TranslateKey(key ,&keysym) );
...
#define SDL_ANDROID_KEYCODE_1 LALT
...
#define SDL_ANDROID_KEYCODE_3 PAGEDOWN
  ...
  keymap[KEYCODE_VOLUME_UP] = SDL_KEY(SDL_ANDROID_KEYCODE_2);
  keymap[KEYCODE_VOLUME_DOWN] = SDL_KEY(SDL_ANDROID_KEYCODE_3)	;
...
{
  fprintf(stderr,"aki key=%08X    action=%80X\n",key,action);
