  ...
  denominator()
{
  ...
  int height = 600; //FIXME: config->screenheight;
    ...
    if (g_config->fullscreen_size.width==1)
    {
      SDL_Rect** modes = SDL_ListModes(NULL, SDL_FULLSCREEN|SDL_HWSURFACE);
      g_config->fullscreen_size = Size(modes[0]->w, modes[0]->h);
    }


  
  screen = SDL_SetVideoMode(g_config->fullscreen_size.width, g_config->fullscreen_size.height, 0, flags);
  if(screen == 0) {
